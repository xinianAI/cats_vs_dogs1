...
PATH = 'D:/cats_vs_dogs/models/model_alexnet.pt'
log_dir = os.path.join('tensorboard', 'valid', 'alexnet')
...
if os.path.exists('models/model_alexnet.pt'):
    net_new = torch.load('models/model_alexnet.pt')
...
optimizer_alexnet = torch.optim.SGD(net_new.parameters(), lr=learning_rate,
                                    momentum=momentum)
...
def train():
    ...
    torch.save(net_new, 'models/model_alexnet.pt')
...
def test():
    ...
    probs_dog = []
    classes = ('cat', 'dog')
    model = torch.load('models/model_alexnet.pt')
    model.eval()
...
def valid():
    ...
    # with open("cat_dog_results/cat_dog_alexnet_result.csv", 'w') as out:
    ...
    y_test = []
    y_score = []
    with open("cat_dog_results/cat_dog_alexnet_result.csv", 'r') as fs:
        for line in fs:
