...
PATH = 'D:/cats_vs_dogs/models/model_densenet.pt'
log_dir = os.path.join('tensorboard', 'valid', 'densenet')
...
# 对加载的图像作归一化处理， 并裁剪为[224x224x3]大小的图像 数据增强
...
net_new.classifier = classifier

if os.path.exists('models/model_densenet.pt'):
    net_new = torch.load('model_densenet.pt')
...
optimizer_densenet = torch.optim.SGD(net_new.parameters(), lr=0.003,
                                     momentum=momentum)
...
def train():
    ...
    torch.save(net_new, 'models/model_densenet.pt')
...
def test():
    ...
    probs_dog = []
    classes = ('cat', 'dog')
    model = torch.load('models/model_densenet.pt')
    model.eval()
    ...
    # plt.boxplot(probs_cat,probs_dog)
    # plt.show()
...
def valid():
    ...
    with open("cat_dog_results/cat_dog_densenet_result.csv", 'w') as out:
    ...
    y_test = []
    y_score = []
    with open("cat_dog_results/cat_dog_densenet_result.csv", 'r') as fs:
        for line in fs:
