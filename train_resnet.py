...
log_dir = os.path.join('tensorboard', 'valid', 'resnet')
...
PATH = 'D:/cats_vs_dogs/models/model_resnet.pt'
...
net_new = net
net_new.fc = nn.Linear(512, 2, bias=True)
net_new = net_new.to(device)

if os.path.exists('models/model_resnet.pt'):
    net_new = torch.load('models/model_resnet.pt')
...
optimizer_resnet = torch.optim.SGD(net_new.parameters(), lr=0.001, momentum=0.9, weight_decay=5e-4)
...
def valid_model(model, dataloader, size):
    ...
    for inputs, classes in dataloader:
        ...
        all_proba[i:i + len(classes), :] = prob[0][0].item() if prob[0][0].item() > prob[0][1].item() else prob[0][
            1].item()
        i += len(classes)
...
def train():
    ...
    torch.save(net_new, 'models/model_resnet.pt')
...
def test():
    ...
    probs_dog = []
    classes = ('cat', 'dog')
    model = torch.load('models/model_resnet.pt')
    model.eval()
    ...
    for i, data in enumerate(test_loader, 0):
        ...
        st = str('%.2f' % float(
            prob[0][0].item() * 100 if prob[0][0].item() > prob[0][1].item() else prob[0][1].item() * 100)
                 + '%')
        if pred_class == 'cat':
            probs_cat.append(
                float(prob[0][0].item() * 100 if prob[0][0].item() > prob[0][1].item() else prob[0][1].item() * 100))
        else:
            probs_dog.append(
                float(prob[0][0].item() * 100 if prob[0][0].item() > prob[0][1].item() else prob[0][1].item() * 100))
...
def valid():
    ...
    # with open("cat_dog_results/cat_dog_resnet_result.csv", 'w') as out:
    ...
    y_test = []
    y_score = []
    with open("cat_dog_results/cat_dog_resnet_result.csv", 'r') as fs:
        for line in fs:
